...
const run = async () => {
    try {
        ...
        if (found)
            console.log('Test client found in list: PASS');
        else
            console.error('Test client NOT found: FAIL');
        // 3. Delete Client
        console.log('Deleting client...');
        await ClientLogo.findByIdAndDelete(client._id);
        const checks = await ClientLogo.findById(client._id);
        if (!checks)
            console.log('Client deleted successfully: PASS');
        else
            console.error('Client still exists: FAIL');
    }
    catch (error) {
