...
export const sendOtpEmail = async ({ to, otp }: { to: string, otp: string }) => {
...
export const sendPlanRequestEmail = async ({ adminEmail, customer, plan }: { adminEmail: string, customer: any, plan: any }) => {
  const transporter = getTransporter();
  const from = process.env.MAIL_FROM || process.env.SMTP_USER;
  const details = [
    `Plan: ${plan.name}`,
