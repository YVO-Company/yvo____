...
const resolveSubscriptionEnd = (billingCycle: any) => {
...
const activateSubscription = async ({ companyId, plan }: any) => {
...
const sendAdminNotice = async ({ user, plan }: any) => {
...
const createStripeSession = async ({ plan, user, companyId, successUrl, cancelUrl }: any) => {
...
const verifyStripeSignature = ({ payload, signatureHeader }: any) => {
  const secret = process.env.STRIPE_WEBHOOK_SECRET;
  if (!secret) {
    throw new Error("Stripe webhook secret not configured.");
  }

  const elements: any = signatureHeader.split(",").reduce((acc: any, item: any) => {
...
export const handleStripeWebhook = async (req: Request, res: Response) => {
  try {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      const payment: any = await Payment.findOne({ providerSessionId: session.id });
