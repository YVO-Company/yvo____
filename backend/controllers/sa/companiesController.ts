...
export const updateCompanyFlags = async (req: Request, res: Response) => {
    try {
        ...
        if (req.body.replace) {
            ...
            company.featureFlags = new Map(Object.entries((flags as any) || {}));
        } else {
            // MERGE (Legacy/Partial update)
            if (company.featureFlags instanceof Map) {
                for (const [key, value] of Object.entries((flags as Record<string, any>) || {})) {
                    company.featureFlags.set(key, value);
                }
            } else {
                company.featureFlags = { ...(company.featureFlags as any), ...((flags as any) || {}) };
            }
        }
    ...
    try {
        ...
        const deletePromises = loadedModels.map(Model => {
            ...
            if (Model.modelName === 'FeatureFlag') {
                return (Model as any).deleteMany({ scope: 'COMPANY', scopeId: companyId });
            }
            return (Model as any).deleteMany({ companyId: companyId });
        });
